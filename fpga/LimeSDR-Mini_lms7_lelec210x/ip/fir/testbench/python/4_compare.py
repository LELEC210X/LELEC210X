...
output_array_fused_q = np.loadtxt("fpga/LimeSDR-Mini_lms7_lelec210x/ip/fir/testbench/mentor/output_fpga.txt",delimiter=" ",converters={0:lambda s: int(s,16)}).astype(np.int32)
...
output_array = np.fromfile(
    "fpga/LimeSDR-Mini_lms7_lelec210x/ip/fir/testbench/mentor/output_float.txt", sep=","
)
...
output_array_fused_q = np.loadtxt(
    "fpga/LimeSDR-Mini_lms7_lelec210x/ip/fir/testbench/mentor/output_fpga.txt",
    delimiter="\n",
    converters={0: lambda s: int(s, 16)},
).astype(np.int32)
output_array_q = np.zeros((2 * len(output_array_fused_q),))
for i in range(len(output_array_fused_q)):
    output_array_q[2 * i] = twos_complement(
        (output_array_fused_q[i] >> nbit) & ((1 << nbit) - 1), nbit
    )
    output_array_q[2 * i + 1] = twos_complement(
        output_array_fused_q[i] & ((1 << nbit) - 1), nbit
    )
...
plot_time(
    ...
    [
        "F32 Real",
        "F32 Imag",
        "Q{}.{} Real".format(nbit - qbit, qbit),
        "Q{}.{} Imag".format(nbit - qbit, qbit),
    ],
    fs,
)
...
plot_freq(
    [
        output_array[:, 0] + 1j * output_array[:, 1],
        output_array_fq[:, 0] + 1j * output_array_fq[:, 1],
    ],
    ["F32", "Q{}.{}".format(nbit - qbit, qbit)],
    fs,
    os=4,
)
>>>>>>> Revert "enlever le chain de argu"
