...
def apply_line(args, i, lines):
    ...
    if args.remove_below:
        j = i+1
        to_reverse = 0
        while j < len(indents) and (indents[j] is None or len(indents[j]) >= len(indents[i])):
...
def outputs_to_markdown(outputs):
    ...
    for output in outputs:
        if output["output_type"] == "display_data" and "image/png" in output["data"]:
            markdown.append("<img src='data:image/jpeg;base64,{}' />".format(output['data']['image/png']))
        elif output["output_type"] == "stream":
            if output["name"] == "stdout":
                markdown.append("<pre>{}</pre>".format(''.join(output["text"])))
            elif output["name"] == "stderr":
                markdown.append("<pre style='color:red;'>{}</pre>".format(''.join(output["text"])))
            else: raise NotImplementedError(output["name"])
        else: raise NotImplementedError(output["output_type"])
