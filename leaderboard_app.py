...
# MODE UTILISATEUR
        ...
        if len(st.session_state.guess_queue) == 0:
            ...
            with col1:
                if st.button("Tout envoyer", use_container_width=True, type="primary", 
                           disabled=st.session_state.auto_submit, key="send_all"):
                    for guess in st.session_state.guess_queue:
                        result = send_request("POST", f"/submit/{api_key}/{guess['value']}", 
            ...
            with col2:
                if st.button("Tout supprimer", use_container_width=True, type="secondary",
                           disabled=st.session_state.auto_submit, key="delete_all"):
                    st.session_state.guess_queue = []
                    st.rerun()
            ...
            for i, guess in enumerate(st.session_state.guess_queue):
                ...
                with col4:
                    if st.button("Suppr.", key=f"del_{guess['id']}", use_container_width=True,
                               disabled=st.session_state.auto_submit):
                        st.session_state.guess_queue.pop(i)
                        st.rerun()
    ...
    with tab2:
        ...
        with col1:
            manual_guess = st.text_input("Guess", placeholder="ex: fire, dog_bark, siren...", key="manual_input")
        
        with col2:
            st.write("")
            st.write("")
            if st.button("Ajouter", use_container_width=True, type="primary", key="add_manual"):
                if manual_guess:
                    guess_obj = {
                        "value": manual_guess,
                    ...
                    if st.session_state.auto_submit:
                        result = send_request("POST", f"/submit/{api_key}/{manual_guess}", 
                                            f"Envoi auto : '{manual_guess}'", show_response=False)
                        if result:
                            st.session_state.total_submitted += 1
                            st.session_state.guess_queue = [g for g in st.session_state.guess_queue 
    ...
    with col1:
        round_num = st.number_input("Round", min_value=0, value=0, step=1, help="Laissez 0 pour tous", key="round_num")
    
    with col2:
        lap_num = st.number_input("Lap", min_value=0, value=0, step=1, help="Laissez 0 pour tous", key="lap_num")
    ...
    with col1:
        st.warning("ATTENTION : Cette action est irréversible ! Utilisez les mêmes filtres Round/Lap que ci-dessus.")
    
    with col2:
        if st.button("Supprimer", use_container_width=True, type="secondary", key="del_subs"):
            endpoint = f"/submissions/{api_key}"
            if round_num > 0:
                endpoint += f"/{round_num}"
                if lap_num > 0:
...
# FOOTER
st.markdown("---")
st.caption(f"LELEC210X Leaderboard Controller | Mode: {st.session_state.mode.upper()} | {base_url}")