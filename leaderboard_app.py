...
with st.sidebar:
    ...
    api_key = st.text_input(
        key_label, type="password", help="La clé générée pour votre groupe"
    )

    if not api_key:
        st.warning("Veuillez entrer une clé pour activer les fonctionnalités.")
...
def send_request(method, endpoint, success_msg="Action réussie", show_response=False):
    ...
    try:
        ...
        st.error("Timeout : Le serveur met trop de temps à répondre")
    except requests.exceptions.ConnectionError:
...
def check_and_add_new_guess(filepath="/tmp/latest_guess.json"):
        ...
        if os.path.exists(filepath):
            ...
            if current_mtime > st.session_state.last_file_mtime:
                ...
                with open(filepath) as f:
                    data = json.load(f)
                    guess_value = data.get("value")
                    guess_time = data.get("timestamp")
                    iso_time = data.get("iso_timestamp")

                if guess_value:
                    guess_id = (
                        f"{guess_value}_{iso_time}"
                        if iso_time
                        else f"{guess_value}_{int(time.time() * 1000)}"
                    )
...
if st.session_state.mode == "admin":
    ...
    if st.button("Actualiser le statut", use_container_width=True):
        status_data = send_request(
            "GET", f"/status/{api_key}", "Statut mis à jour", show_response=True
        )
    ...
    with col_restart:
        if st.button("RESTART", use_container_width=True, type="secondary"):
            st.warning("Confirmez le redémarrage ci-dessous")
    ...
    with st.form("rename_form"):
        ...
        if submitted and new_name:
            ...
            send_request(
                "PATCH",
                f"/rename/{api_key}/{safe_name}",
                f"Équipe renommée en '{new_name}'",
            )
    ...
    if st.button("Obtenir les résultats", use_container_width=True):
        results = send_request(
            "POST", f"/results/{api_key}", "Résultats récupérés", show_response=True
        )

# MODE UTILISATEUR
else:
    # Vérifier les nouveaux guesses
    file_path = "/tmp/latest_guess.json"
    new_guess = check_and_add_new_guess(file_path)
    ...
    # EN MODE AUTO : envoyer tous les guesses
    if st.session_state.auto_submit:
        st.success("Mode automatique - Les guesses sont envoyés dès leur détection")

        guesses_to_remove = []
        for guess in st.session_state.guess_queue:
            result = send_request(
                "POST",
                f"/submit/{api_key}/{guess['value']}",
                f"Envoi auto : '{guess['value']}'",
                show_response=False,
            )
            if result:
        ...
        if guesses_to_remove:
            st.session_state.guess_queue = [
                g
                for g in st.session_state.guess_queue
                if g["id"] not in guesses_to_remove
            ]
    else:
        st.info("Mode manuel - Choisissez quels guesses envoyer")

    st.markdown("---")

    # Tabs
    tab1, tab2 = st.tabs(["File d'attente", "Ajout manuel"])
    ...
    with tab2:
        ...
        with col1:
            manual_guess = st.text_input(
                "Guess", placeholder="ex: fire, dog_bark, siren...", key="manual_input"
            )
        ...
        with col2:
            ...
            st.write("")
            if st.button(
                "Ajouter", use_container_width=True, type="primary", key="add_manual"
            ):
                if manual_guess:
                    guess_obj = {
                        "value": manual_guess,
                    ...
                    st.session_state.guess_queue.append(guess_obj)
                    st.success(f"Guess '{manual_guess}' ajouté")

                    if st.session_state.auto_submit:
                        result = send_request(
                            "POST",
                            f"/submit/{api_key}/{manual_guess}",
                            f"Envoi auto : '{manual_guess}'",
                            show_response=False,
                        )
                        if result:
                            st.session_state.total_submitted += 1
    ...
    with col3:
        st.write("")
        st.write("")
        get_btn = st.button("Récupérer", use_container_width=True, key="get_subs")
    ...
    if get_btn:
        ...
        result = send_request(
            "GET", endpoint, "Soumissions récupérées", show_response=True
        )
    ...
    with col1:
        st.warning(
            "ATTENTION : Cette action est irréversible ! Utilisez les mêmes filtres Round/Lap que ci-dessus."
        )

    with col2:
        if st.button(
            "Supprimer", use_container_width=True, type="secondary", key="del_subs"
        ):
            endpoint = f"/submissions/{api_key}"
            if round_num > 0:
                endpoint += f"/{round_num}"
            ...
            result = send_request(
                "DELETE", endpoint, "Soumissions supprimées", show_response=True
            )
...
st.markdown("---")
st.caption(
    f"LELEC210X Leaderboard Controller | Mode: {st.session_state.mode.upper()} | {base_url}"
)
