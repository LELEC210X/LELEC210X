...
class SignalProcessor:
    ...
    def _validate_data(self) -> None:
        if len(self.signal_data.raw_signal) != len(self.signal_data.time):
            raise ValueError("Signal and time arrays must have same length")
        if not isinstance(self.signal_data.raw_signal, np.ndarray):
            self.signal_data.raw_signal = np.array(self.signal_data.raw_signal)
    ...
    def process_signal(self, 
        ...
        try:
            ...
            processed, time = self._trim_signal(processed, time, start_trim, end_trim)
            processed, time = self._apply_offsets(processed, time, voltage_offset, time_offset)
            processed, time = self._apply_scaling(processed, time, voltage_scale, time_scale)
