...
def gen_mel_header(f_sample:int, num_fft:int, num_mel:int, header_file:str):
    ...
    header_lines = []
    header_lines += [f"// Mel filter bank header for {f_sample}Hz sample rate, {num_fft} FFT size and {num_mel} mel bins",
                     f"// Triangle struct with {triangle_max_len} maximum values"]
