...
def plot_signal_and_fft(signal, sampling_rate):
    ...
    fig = make_subplots(rows=2, cols=1, subplot_titles=("Time-Domain Signal", "Signal FFT"))
        ...
        with serial.Serial(port, baudrate) as ser:
            ...
            while not stop_event.is_set():
                ...
                if line.startswith(PRINT_PREFIX):
                    ...
                    print_and_save_data(f">Aquisition Memory: {len(line) - len(PRINT_PREFIX)} bytes")
                    print_and_save_data(f">Aquisition Size  : {len(buffer_array)} samples")
...
class UARTReaderApp:
    ...
    def create_widgets(self):
        ...
        ttk.Label(credits_frame, text="UART Reader Application for LELEC210x", font=("Helvetica", 16)).grid(row=0, column=0, sticky=tk.W)
        ttk.Label(credits_frame, text="Developed by Group E 2024", font=("Helvetica", 7)).grid(row=1, column=0, sticky=tk.W)
        ttk.Label(credits_frame, text="How to use: Select COM port and click Connect", font=("Helvetica", 10)).grid(row=3, column=0, sticky=tk.W)
        ...
        ttk.Entry(frame, textvariable=self.baudrate_var).grid(row=2, column=1, sticky=(tk.W, tk.E))

        ttk.Label(frame, text="Frequency Sampling:").grid(row=3, column=0, sticky=tk.W)
        ttk.Entry(frame, textvariable=self.freq_sampling_var).grid(row=3, column=1, sticky=(tk.W, tk.E))

        ttk.Label(frame, text="Max ADC Value:").grid(row=4, column=0, sticky=tk.W)
        ttk.Entry(frame, textvariable=self.val_max_adc_var).grid(row=4, column=1, sticky=(tk.W, tk.E))

        ttk.Label(frame, text="VDD:").grid(row=5, column=0, sticky=tk.W)
        ttk.Entry(frame, textvariable=self.vdd_var).grid(row=5, column=1, sticky=(tk.W, tk.E))

        ttk.Checkbutton(frame, text="Enable Logging", variable=self.log_var).grid(row=6, column=0, columnspan=2, sticky=tk.W)
        ttk.Checkbutton(frame, text="Auto Delete Audio Files", variable=self.auto_delete_audio_var).grid(row=6, column=1, columnspan=2, sticky=tk.W)
        ...
        self.console = scrolledtext.ScrolledText(frame, wrap=tk.WORD, width=50, height=10)
    ...
    def connect(self):
        ...
        if self.current_port and self.current_port != port:
            if not messagebox.askyesno("Switch Connection", f"Do you want to switch connection to {port}?"):
                return
                ...
                try:
                    ...
                    if line:
                        ...
                        if buffer is not None:
                            ...
                            self.print_stuff(f">Aquisition Memory\t: {len(line) - len(PRINT_PREFIX)} bytes")
                            self.print_stuff(f">Aquisition Size  \t: {len(buffer_array)} samples")
                            plot_signal_and_fft(buffer_array, FREQ_SAMPLING)
                except Exception as e:
                    self.print_stuff(f"Error: {e}", "error")
                    self.disconnect()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="UART Reader Application")
    parser.add_argument("-p", "--port", help="Port for serial communication")
    parser.add_argument("-b", "--baudrate", help="Baudrate for serial communication")
    parser.add_argument("-f", "--freq-sampling", help="Sampling frequency")
    parser.add_argument("-m", "--max-adc", help="Max ADC value")
    parser.add_argument("-v", "--vdd", help="VDD value")
    ...
    if args.port:
        ...
        print("Console mode activated, to stop, press 'Ctrl+C' then wait for a new serial input to close\n")
        if args.baudrate: baudrate = int(args.baudrate)
        if args.freq_sampling: FREQ_SAMPLING = int(args.freq_sampling)
        if args.max_adc: VAL_MAX_ADC = int(args.max_adc)
        if args.vdd: VDD = float(args.vdd)
        if args.log: LOG_ENABLED = True
        if args.auto_delete: AUTO_DELETE_AUDIO = True
        if args.gui: launch_gui()
        else :
            # Console mode: start reading serial data from the specified port
            read_serial(args.port.strip())
