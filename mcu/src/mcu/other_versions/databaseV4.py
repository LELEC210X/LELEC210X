    ...
    def run_callbacks(self) -> None:
        ...
        with self._lock:
            if self._print_on_trigger:
                logger.debug(f"Running {len(self._callbacks)} callbacks for {self.definition.id}")
...
class EntryContainer:
    ...
    def __str__(self) -> str:
        ...
        message+= f"> Description: {self.definition.description}\n"
        message+= f"> Entries: {[entry.definition.id for entry in self.entries.values()]}\n"
...
class Database:
    ...
    def __init__(self):
        self.root_container = EntryContainer(ContainerDefinition("root", "Root", "Root container"))
    ...
    def _decode_container_path(self, container_path: str) -> List[str]:
        ...
        path_elements = [element.strip() for element in path_elements if element.strip()]
    ...
    def add_container(self, container_path: str, definition: ContainerDefinition) -> EntryContainer:
        """
    ...
    def add_entry(self, container_path: str, definition: EntryDefinition) -> DatabaseEntry:
        """
    ...
    def __str__(self) -> str:
        ...
        def print_container(container: EntryContainer, level: int) -> str:
            ...
            message+= f"{SPACER*level}> Description: {container.definition.description}\n"
...
class ConfigAPI:
    ...
    def create_container(
        ...
        return self._db.root_container.add_container(EntryContainer(ContainerDefinition(container_id, name, description)))
...
def main():
    ...
    # Create a container
    hidden_class = database.add_container("root", ContainerDefinition(
        id="hidden_class",
        name="Hidden Class",
    ...
    hidden_class.add_entry(DatabaseEntry(EntryDefinition(
        id="app_version",
