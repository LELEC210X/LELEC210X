...
static q15_t buf_fft[2*SAMPLES_PER_MELVEC  ]; // Double size (real|imag) buffer needed for arm_rfft_q15
static q15_t buf_tmp[  SAMPLES_PER_MELVEC/2]; // Intermediate buffer for arm_mat_mult_fast_q15


/*
 * @brief  Compute a mel-vector from raw audio samples.
 * @param[in]   samples    sample array of length SAMPLES_PER_MELVEC
 * @param[out]  melvec     melvec array of length MELVEC_LENGTH
 * @note  Samples are modified in-place by this function.
 */
void melvec_compute(q15_t *samples, q15_t *melvec)
{
	// STEP 0   : Convert 12-bit DC ADC samples to Q1.15 fixed point signal and remove DC component
	// STEP 0.1 : Increase fixed-point scale
	//            --> Pointwise shift
	//            Complexity: O(N)
	//            Number of cycles: <TODO>

	// The output of the ADC is stored in an unsigned 12-bit format, so samples[i] is in [0 , 2**12 - 1]
	...
	// That way, the value of samples[i] is in [0 , 2**15 - 1]
	...
	arm_shift_q15(samples, 3, samples, SAMPLES_PER_MELVEC);
	...
	//           --> We only keep positive frequencies (symmetry) in the next operations.
	//           Complexity: O(Nlog(N))
	//           Number of cycles: <TODO>
...
#if (DEBUGP == 1)
	float y;
	DEBUG_PRINT("Acquisition complete, the following mel-vectors were computed:\r\n");
	for(unsigned int j=0; j < N_MELVECS; j++) {
		DEBUG_PRINT("MV #%u:\t", j+1);
		for(unsigned int i=0; i < MELVEC_LENGTH; i++) {
			arm_q15_to_float(&(mel_vectors[j][i]), &y, 1);
			DEBUG_PRINT("%.3f, ", y);
		}
		DEBUG_PRINT("\r\n");
	}
