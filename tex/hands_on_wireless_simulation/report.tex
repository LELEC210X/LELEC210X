...
\begin{enumerate}
    ...
    \begin{itemize}
        \item a BER-$\sfrac{\varepsilon}{N_0}$ curve with perfect synchronization, showing the performance of your demodulation algorithm (all bypass boolean flags should be set to \py{True});
        \item a BER-$\sfrac{\varepsilon}{N_0}$ curve of the complete chain, involving all the synchronization blocks (all bypass boolean flags set to \py{False}, except for the ideal preamble that we recommend setting to \py{True}).
    \end{itemize}
    ...
    \item The simulated RMSE curve of the CFO estimation (with a random CFO generated from \SIrange{-1}{1}{\kilo\hertz}, all bypass boolean flags set to \py{False}, except for the ideal preamble that we recommend setting to \py{True}). Briefly explain the different trends observed in the curve.
\end{enumerate}
