...
This simulated BER vs $\sfrac{\varepsilon}{N_0}$ curve can then be compared to some theoretical curves, obtained thanks to mathematical developments based on the decision variable and the model of the noise (AWGN). For example, the bit error probability in the case of a BPSK transmission is given by
\begin{equation*}
...
You now have all the necessary explanations to understand the generation of BER vs $\sfrac{\varepsilon}{N_0}$ curves by the simulation framework and compare them with the theory. In \textbf{chain.py}, make sure all the booleans bypassing (i.e., \py{ideal_* = True}) the synchronization are set to \py{True}. Then, run the file and observe the generated BER vs $\sfrac{\varepsilon}{N_0}$ curves. Do they stick to the theoretical curves?
...
To test your implementation, first use \textbf{test\_chain.py} and make sure the estimated CFO corresponds to the applied one. You can then use the \textbf{chain.py} file, setting the CFO bypass boolean to \py{False}. Look at the RMSE, does it decrease with larger $\sfrac{\varepsilon}{N_0}$?
...
Set the bypassing boolean of the symbol timing estimation to \py{False} in \textbf{chain.py}. Launch the simulation and observe the symbol timing RMSE automatically displayed. Does it decrease with increasing $\sfrac{\varepsilon}{N_0}$?
