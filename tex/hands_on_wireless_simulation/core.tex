...
Complete the \py{demodulate()} function in the file \textbf{chain.py} based on the development presented above (non-coherent detection) and the comments.
...
\subsection{BER vs $\protect \bm{\varepsilon / N_0}$ curve}
...
\begin{bclogo}[couleur = gray!20, arrondi = 0.2, logo=\bccrayon]{Simulation of BER vs $\protect \varepsilon / N_0$ curve}
You now have all the necessary explanations to understand the generation of BER vs $\varepsilon / N_0$ curves by the simulation framework and compare them with the theory. In \textbf{chain.py}, make sure all the booleans bypassing (i.e., \py{ideal_* = True}) the synchronization are set to \py{True}. Then, run the file and observe the generated BER vs $\varepsilon / N_0$ curves. Do they stick to the theoretical curves?
...
If needed, you can change the range of $\varepsilon / N_0$ as well as the number of packets and the number of bits per packet in \textbf{chain.py}.
\end{bclogo}
...
\begin{bclogo}[couleur = gray!20, arrondi = 0.2, logo=\bccrayon]{Simulation of PER vs $\protect \varepsilon / N_0$ curve}
...
To test your implementation, first use \textbf{test\_chain.py} and make sure the estimated CFO corresponds to the applied one. You can then use the \textbf{chain.py} file, setting the CFO bypass boolean to \py{False}. Look at the RMSE, does it decrease with larger $\varepsilon / N_0$?
...
Set the bypassing boolean of the symbol timing estimation to \py{False} in \textbf{chain.py}. Launch the simulation and observe the symbol timing RMSE automatically displayed. Does it decrease with increasing $\varepsilon / N_0$?
