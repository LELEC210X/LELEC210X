...
\begin{tabular}{lccl}
    ...
    \texttt{emitter\_id} & 1 & & Unique id of the sensor node.\\
...
At the end of the semester, you will have to characterize your system. Regarding the MCU, you might want to have an insight on the number of cycle of a given task, as well as the memory footprint of the code (in the flash memory) and the heap and stack (in the RAM memory). We explained in \emph{H6a} how to measure the number of cycles. You can find information about the memory footprint in the \emph{Build Analyser} tab in STM32CubeIDE.

Additionally, when compiling, several optimizations steps are provided by the compiler. Those are grouped into optimization levels for ease of use. The optimizations re-organise the low-level operations while keeping the same final code functionality. Depending on the chosen level, you can have a different trade-off between code size, speed (number of cycles) and debug capabilities.
