...
This document contains guidelines for the characterization of telecommunication systems. Please use the main guide for the other parts of the characterization.
...
You have access to all the parameters in the {eval\_radio.h} file: 
\begin{itemize}
    \item \textit{N\_PACKETS} : number of packets sent with an equal TX power each time you press the blue button,
    \item \textit{PAYLOAD\_LEN} : size of the packet payload in bytes,
    \item \textit{PACKET\_DELAY} : time between two transmissions \textbf{in milliseconds. Do not go below 100 milliseconds},
    \item \textit{BLINK\_LED} : a LED is initially toggled after each transmission. Specify 0 to deactivate this feature which will be annoying if you use small value for the delay between to packets,
    \item \textit{STEP\_PA\_LEVEL} : value of the transmit power increase each time the blue button is pressed. Specify 0 to keep the \textit{MIN\_PA\_LEVEL} everytime,
    \item \textit{MIN\_PA\_LEVEL} : initial value of the transmit power,
    \item \textit{MIN\_PA\_LEVEL} : stop value of the transmit power.
\end{itemize}
...
To obtain the experimental packet error rate (PER) as a function of the signal-to-noise ratio (SNR), one must measure the number of packets incorrectly demodulated by GNU Radio at several SNRs. To characterize the demodulator and the synchronization blocks without being impacted by others impairements, we developed a measurement setup in GNU Radio in which a AWGN is injected with a controlled power (N0). This allows to set only once the RX gain, and to then modify the SNR, or $E_s/N_0$, by changing the injected noise power. Moreover, this ease the detection of the packets which is done by the packet presence detector (PPD) before the addition of the AWGN. The flowgraph is named \textit{eval\_error\_rate.grc}. On the left side, you can see the AWGN that is generated in the block \textit{"Noise source"} and added to the signal captured by the LimeSDR. 
...
\begin{itemize}
    \item \textit{rx\_gain} : LimeSDR gain  in the receiver path, in dB,
    \item \textit{noise\_pow\_dB} : power of the AWGN added to the signal, in dB,\footnote[1]{In the GUI, there should be a mentioned of \textit{Noise\_amplitude} which is the converted value from noise power in dB to amplitude in linear scale.}
    \item \textit{K\_threshold} : K factor of the packet presence detector in the FPGA.
\end{itemize}

Moreover, you have a series of check boxes : 
\begin{itemize}
    \item \textit{Print\_payload} : print the demodulated payload of the packets,
    \item \textit{Print\_metrics} : print the estimated CFO, STO and $E_s/N_0$ of the received packets, 
    \item \textit{Print\_results} : print the count of successfully demodulated packets,
    \item \textit{Enable\_detection} : activates the chain by enabling PPD on the FPGA, 
    \item \textit{Enable\_FPGA\_FIR} : activates the FIR on the FPGA.
\end{itemize}

To minimize the computational load of the flowgraph, you should deactivate the \textit{QT GUI Sink} block by clicking on it and pressing \textbf{D} (you can reactivate it by pressing \textbf{E}). Do not forget to regenerate the flow graph afterwards (Cube icon). The low pass filter present on GNU Radio should already be deactivated, as we ask you to use the FIR on the FPGA for the characterization report R6.

When you are ready to start a batch measurement, follow this procedure:
\begin{enumerate}
    \item Launch the GNU Radio flow graph \textit{eval\_error\_rate.grc}.
    \item Update the \textit{noise\_pow\_dB} so that it is some order of magnitude above the quantization noise. The \textit{Noise\_amplitude} is displayed in the GUI to guide you. You should be higher than -40 dB (0.01 in linear scale).
    \item Enable the detection of the packets (Check \textit{Enable\_detection}) and the FPGA FIR (\textit{Enable\_FPGA\_FIR}).
    \item Change the RX gain and start sending some packets. The average measured signal amplitude is displayed in the console ("\textit{Avg Ampltiude XX}") and indicates if you are close to the saturation, which must be avoided. 
    \item Modify \textit{noise\_pow\_dB} so that the measured $E_s/N_0$ (in the console) is close to the one you want to measure.\footnote[2]{If you specified a STEP\_PA\_LEVEL different from 0 on the MCU, remember that the transmit power, and thus $E_s/N_0$, will be modified everytime you press the blue button of the Nucleo.}
