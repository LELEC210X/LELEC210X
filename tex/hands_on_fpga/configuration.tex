...
\hline
\texttt{cfg\_preamble\_en} & Enable or disable the PPD. & $\left\{0,1\right\}$ \\
\hline
\texttt{cfg\_clear\_rs} & Clear the delay lines of the dual running sum. & $\left\{0,1\right\}$ \\
\hline
...
\hline
\texttt{cfg\_fir\_en} & Enable the FIR present in the FPGA. & $\left[1,2^{8}-1\right]$ \\
\hline
\texttt{cfg\_pass\_sum\_signal} & Bypass signal to forward short and long sum results. & $\left\{0,1\right\}$ \\
\hline
\texttt{cfg\_red\_sum\_signal} & Reduce the forwarded short and long sum signals by applying a rightshift by 6. & $\left\{0,1\right\}$ \\
\hline
\end{tabular}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/design_hierarchy_fpgacfg_both.PNG}
